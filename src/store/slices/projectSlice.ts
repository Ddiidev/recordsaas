...
import type { MetaDataItem, ZoomRegion, CursorFrame, FrameStyles } from '../../types'
...
const reconcileAutoZoomRegionsForTakes = (state: AutoZoomTakeState): void => {
  ...
  const previousAutomaticZoomRegions = Object.fromEntries(
    Object.entries(state.zoomRegions).filter(([, region]) => isAutoGeneratedZoomRegion(region)),
  )
  ...
  const automaticZoomRegions = state.takeModeEnabled
  ...
  state.zoomRegions = {
    ...manualZoomRegions,
    ...Object.fromEntries(
      Object.entries(automaticZoomRegions).map(([id, region]) => {
        const previous = previousAutomaticZoomRegions[id]
        return [
          id,
          previous
            ? {
...
const resolveProjectBackgroundImage = (
  frameStyles: FrameStyles,
  projectFolder: string | null | undefined,
): FrameStyles => {
  if (frameStyles.background.type !== 'image' || !frameStyles.background.imagePath) return frameStyles

  const imagePath = resolveProjectMediaPath(projectFolder, frameStyles.background.imagePath)
  ...
  return {
    ...
    background: {
      ...
      imagePath,
      imageUrl: toMediaUrl(imagePath) || frameStyles.background.imageUrl,
    },
  }
}
  ...
  loadProject: async ({
    ...
    try {
      ...
      set((state) => {
        ...
        if (frameStyles) {
          state.frameStyles = resolveProjectBackgroundImage(frameStyles, state.originalProjectPath)
        }
