...
export const WebcamCropEditor = memo(
  ({ sourceUrl, sourceKind = 'video', currentTime, crop, onUpdateCrop }: WebcamCropEditorProps) => {
    ...
    const renderFrame = useCallback(() => {
      ...
      const source = sourceKind === 'image' ? image : video
      const sourceWidth = sourceKind === 'image' ? image?.naturalWidth : video?.videoWidth
      const sourceHeight = sourceKind === 'image' ? image?.naturalHeight : video?.videoHeight
      if (!canvas || !source || !sourceWidth || !sourceHeight) return
    ...
    useEffect(() => {
      ...
      renderFrame()
    }, [currentTime, renderFrame, sourceKind])
    ...
    return (
      <div className="space-y-3">
        <div className="rounded-lg border border-border/70 bg-muted/30 p-3 text-xs text-muted-foreground">
          Frame atual da fonte. Arraste os 6 pontos para recortar.
        </div>
        ...
        >
          ...
          {sourceKind === 'image' ? (
            <img ref={imageRef} src={sourceUrl} alt="" className="hidden" />
          ) : (
            <video ref={videoRef} src={sourceUrl} muted playsInline preload="auto" className="hidden" />
          )}
            ...
            {HANDLE_CONFIG.map(({ handle, cursor, getPosition }) => {
              ...
              return (
                <button
                  ...
                  aria-label={`Ajustar crop ${handle}`}
                />
              )
            })}
          </div>
